...
async def move(ctx, queue, *args):
    ...
    elif len(args) == 1: await move_to(ctx, queue, args[0], 1)
    elif len(args) == 2: await move_to(ctx, queue, args[0], args[1])
    else: await embedded_message(ctx, ":japanese_goblin: **Invalid Syntax**", "_Function only takes two arguments_")
...
async def move_to(ctx, queue, pos1, pos2):
    ...
    if pos1 == pos2:
        await embedded_message(ctx, ":japanese_goblin: **Invalid Position**", f"_Song is already in position_ `{pos1}`")
        return
